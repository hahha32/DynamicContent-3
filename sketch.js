const commentatorColors = {
  "CAWylie": "#FF0000", "Rahcmander": "#800080", "TheJoebro64": "#BA55D3",
  "I am RedoStone": "#87CEEB", "Igordebraga": "#FFFF00", "Vestrian24Bio": "#0000FF",
  "Bucket of sulfuric acid": "#00008B", "Benmite": "#808080", "Shuipzv3": "#006400"
};

const toppingsMap = {
  "List-Class": "sprinkles", "B-Class": "chocolate chips",
  "Good": "caramel drizzle", "Featured": "whipped cream"
};

const conesMap = {
  "Jan": "waffle", "Feb": "waffle", "Mar": "sugar", "Apr": "sugar",
  "May": "cake", "Jun": "cake", "Jul": "wafer", "Aug": "wafer",
  "Sept": "waffle", "Oct": "sugar", "Nov": "cake", "Dec": "wafer", "ul": "wafer"
};
...
function draw() {
  ...
  fill(0);
  textSize(12);
  text(`Filters: Size=${filters.size}, Topping=${filters.topping}, Cone=${filters.cone}`, 10, height - 10);
}

function getFilteredData() {
  return data.filter(item => {
    const sizeMatch = filters.size === "any" || 
      getSizeCategory(item.scoopSize) === filters.size;
    const toppingMatch = filters.topping === "any" || item.topping === filters.topping;
...
function drawLegend() {
  ...
  fill(255, 230);
  stroke(0);
  rect(10, 70, 200, Object.keys(commentatorColors).length * 20 + 20);
...
function drawIceCreamCone(x, y, item) {
  ...
  const coneColors = {
    "waffle": "#f4a460", "sugar": "#f5deb3",
    "cake": "#d2b48c", "wafer": "#e6d5b8"
  };
  
  fill(coneColors[item.coneType]);
  if (item.coneType === "wafer") {
    rectMode(CENTER);
    rect(0, 60, item.scoopSize * 0.7, 120, 5);
  } else {
    beginShape();
    vertex(-item.scoopSize * 0.4, 0);
    vertex(0, 120);
    vertex(item.scoopSize * 0.4, 0);
    endShape(CLOSE);
    
    if (item.coneType === "waffle") {
      stroke(200, 150, 100);
      for (let i = 0; i < 5; i++) {
        line(-item.scoopSize * 0.4 + 10, map(i, 0, 4, 0, 120), 
             item.scoopSize * 0.4 - 10, map(i, 0, 4, 0, 120));
      }
      noStroke();
    }
  }
  ...
  if (dist(mouseX, mouseY, x, y - item.scoopSize/2) < item.scoopSize/2 + 20) {
    info(item);
  }
  pop();
}
...
function drawTopping(x, y, size, type) {
  ...
  const toppings = {
    "sprinkles": { color: [255, 0, 0], count: 20, draw: (x, y) => rect(x, y, 2, 6) },
    "chocolate chips": { color: [101, 67, 33], count: 15, draw: (x, y) => ellipse(x, y, 6, 4) },
    "caramel drizzle": { 
      color: [210, 130, 50], draw: () => {
    ...
    "whipped cream": {
      color: [255], draw: () => {
        ...
        for (let angle = 0; angle < TWO_PI; angle += PI/4) {
          ellipse(cos(angle) * size * 0.35, sin(angle) * size * 0.35 - size * 0.1, 15, 20);
        }
      }
    }
  };
